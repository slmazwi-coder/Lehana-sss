...
- School fees — Lehana is a Quintile 1 No-Fee School
...
School details:
- Name: Lehana Senior Secondary School
- EMIS Number: 200501358
- Date Established: 1 January 1957
- Location: Castle Rock, Tsekong Location (Ward 8), Mount Fletcher, 4770 (Eastern Cape)
- Postal Address: P.O. Box 45, Mount Fletcher, 4770
- Phone: 072 119 6510 / 039 257 9021
- Email: 200501358@ecschools.org.za
...
- 2021 Matric pass rate: 67.2% (266 out of 396 learners passed)
...
async function askClaude(userMessage: string): Promise<string> {
  try {
    ...
    return 'I\'m having trouble connecting right now. Please contact the school directly at 072 119 6510 or 200501358@ecschools.org.za.';
  }
}
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      role: 'bot',
      createdAt: Date.now(),
      text: "👋 Hello! Let me help you! Whether it's admissions, boarding, fees, results, activities or anything else about Lehana SSS — just ask and I'll be happy to assist.",
    },
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: uid(),
          role: 'bot',
          text: 'Something went wrong. Please contact the school at 072 119 6510.',
          createdAt: Date.now(),
        },
      ]);
    } finally {
  ...
  return (
    <>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#D4A017] text-white shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-bold text-sm leading-tight truncate">Lehana Assistant</div>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-gray-50 scroll-smooth">
            {messages.map((m) => (
              ...
              >
                {m.role === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-[#D4A017] flex items-center justify-center shrink-0 mb-0.5">
            ...
            {isTyping && (
              <div className="flex items-end gap-1.5">
                <div className="w-6 h-6 rounded-full bg-[#D4A017] flex items-center justify-center shrink-0">
                  <Sparkles size={11} className="text-white" />
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {QUICK_QUESTIONS.map((q) => (
                    <button
                      key={q}
                      onClick={() => send(q)}
                      className="text-[11px] px-2.5 py-1 rounded-full bg-white border border-[#1B3A5C]/25 text-[#1B3A5C] hover:bg-[#B89200] hover:text-white transition-colors font-medium shadow-sm"
                    >
                      {q}
                    </button>
                  ))}
          ...
          <div className="p-3 bg-white border-t border-gray-100 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#D4A017]/20 focus:border-[#1B3A5C]/40 transition-all bg-gray-50 placeholder:text-gray-400"
              ...
              <button
                ...
                className="bg-[#D4A017] hover:bg-[#B89200]/90 text-white px-3 py-2 rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shrink-0"
