...
const inp = 'border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#D4A017]/40 focus:border-[#8B1A1A] transition w-full bg-white';
...
const SectionHeading = ({ title }: { title: string }) => (
  <div className="flex items-center gap-2 pb-2 border-b-2 border-[#8B1A1A]/20 mb-5">
    <FileText size={15} className="text-[#8B1A1A] shrink-0" />
...
const StepBadge = ({ num, label, active, done }: {
  ...
  <div className="flex items-center gap-2">
    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black border-2 transition-all
      ${done ? 'bg-[#D4A017] border-[#8B1A1A] text-white'
      : active ? 'bg-white border-white text-[#8B1A1A]'
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-xl border-2 transition
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={`text-xs mt-0.5 truncate ${file ? 'text-[#8B1A1A]' : 'text-gray-400'}`}>
          {file ? file.name : 'No file chosen'}
        </div>
      </div>
      <label className="shrink-0 text-xs font-bold text-[#8B1A1A] cursor-pointer hover:underline">
        {file ? 'Change' : 'Upload'}
  ...
  if (submitted) {
    return (
      <div className="py-20 flex items-center justify-center min-h-[60vh]">
        ...
        >
          <div className="w-20 h-20 bg-[#FFF5D6] text-[#8B1A1A] rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle size={48} />
          </div>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr>
                              ...
                              <th className="text-left px-3 py-2 bg-[#FDF8E8] text-xs font-bold text-[#8B1A1A] uppercase">Father / Guardian / Responsible Person</th>
                              <th className="text-left px-3 py-2 bg-[#FDF8E8] text-xs font-bold text-[#8B1A1A] uppercase">Mother / Relative</th>
                            </tr>
                          </thead>
                          <tbody>
                            {([
                  ...
                  <>
                    ...
                    <section>
                      ...
                      <button type="button" onClick={() => setChildren(p => [...p, { surname:'',firstName:'',dob:'',grade:'',school:'' }])}
                        className="mt-2 text-xs text-[#8B1A1A] font-semibold hover:underline">+ Add another child</button>
                    </section>
                  ...
                  <>
                    ...
                    <section className="bg-[#FDF8E8] border-2 border-[#D4A017] rounded-2xl p-6">
                      <div className="flex gap-4 items-start">
                        <div className="w-12 h-12 bg-[#FFF5D6] text-[#8B1A1A] rounded-xl flex items-center justify-center shrink-0">
                          <Download size={22} />
                        </div>
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-[#8B1A1A] mt-3">
